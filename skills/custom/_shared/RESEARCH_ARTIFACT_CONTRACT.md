...
除独立的 `handoff` 外，其余四个自定义 SKILL 共用同一套项目状态和证据记录，避免每个工具维护一份互不相通的上下文。`handoff` 可以把这些记录作为普通的可选来源，但不依赖本约定。
...
4. **可选任务工作集**：独立 `handoff` 默认写入 `.handoffs/*.md`，为特定接收方和成功标准裁剪临时证据包。
