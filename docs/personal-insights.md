...
再比如 Phase C 的优先级规则：高成本实验留给人工跟进、超过 30 分钟的实验放到后台并继续其他修复、优先 reframe 而非新实验、总是实现 metric additions（成本低、收益高）。这些是调度原则，不是计算预算硬闸门；真实预算仍需由项目单独给出。
...
所有循环类 workflow 都面临“什么时候停”的问题——auto-review-loop 用 score 阈值，但分数是 GPT 的主观判断；AI4AI 则把停止写进冻结规格，用 trial、wall time、GPU/CPU 时间、费用、目标指标和 patience 机械判断。`resume` 只能在核对版本与剩余预算后继续，`report` 不得产生新实验。
